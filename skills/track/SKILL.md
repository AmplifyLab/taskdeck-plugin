...
0. Make sure TaskDeck is connected. If the TaskDeck tools (`who-am-i`, `track-repository`) are available in this session, go to step 1. If they are not:
   - Add TaskDeck to Claude Code's MCP servers yourself by editing the file, not with the `claude` command: Claude Desktop has none. If `~/.claude.json` has no `taskdeck` server under `"mcpServers"`, add one with `"type": "http"` and `"url": "https://mcp.taskdeck.me/mcp"`, and change nothing else in that file.
   - A session does not load a server added while it is open. Tell the person in one sentence: open a new session, run /mcp there to sign in to TaskDeck, then run /taskdeck:track again. Stop here.
